...
strategy_metrics = {
    ...
    'breakout_extended_squeeze_usage': 0,
    'breakout_layer3_fail': 0,
    'breakout_layer4_fail': 0,
    'breakout_layer5_fail': 0,
    'pullback_vwap_extended_usage': 0,
    # Öneri metriği: L1 başarısız ama 4H momentum çok güçlü → scalp fallback önerildi
    'breakout_suggest_scalp_fallback': 0
}
...
def _maybe_dump_metrics(config):
    ...
    if now - _last_metrics_dump_ts >= interval:
        msg = (
            ...
            f"BRK_EXT_SQZ={strategy_metrics['breakout_extended_squeeze_usage']} "
            f"BRK_SCALP_FB_SUGGEST={strategy_metrics['breakout_suggest_scalp_fallback']}"
        )
        logger.info(msg)
...
def check_institutional_trend_1d(df_1d: pd.DataFrame) -> tuple:
    ...
    last = df_1d.iloc[-1]
    # NaN remediation: son bar'da NaN varsa bir önceki bar ile forward-fill dene
    if last[required].isna().any():
        if len(df_1d) >= 2:
            prev = df_1d.iloc[-2]
            filled = last.copy()
            for col in required:
                if pd.isna(filled[col]) and not pd.isna(prev[col]):
                    filled[col] = prev[col]
            # Eğer hala NaN varsa başarısız say
            if filled[required].isna().any():
                return None, "1D'de NaN değer var (prev fallback başarısız)"
            last = filled
        else:
    ...
    # Semi-hierarchy (gevşek kabul):
    # - Bullish: backbone ema20 > ema50 > sma200 ve close > ema20; ema5, ema20'nin %0.5 altına sarkabilir
    # - Bearish: backbone ema20 < ema50 < sma200 ve close < ema20; ema5, ema20'nin %0.5 üstüne sarkabilir
    tol = 0.005
    semi_bullish = (ema20 > ema50 > sma200) and (close > ema20) and (ema5 >= ema20 * (1 - tol))
    semi_bearish = (ema20 < ema50 < sma200) and (close < ema20) and (ema5 <= ema20 * (1 + tol))
    ...
    if bullish_hierarchy:
        ...
        # Semi-hiyerarşi kabulü
        if semi_bullish:
            return 'LONG', (
                f"1D semi bullish hierarchy (Close>{ema20:.2f}>{ema50:.2f}>{sma200:.2f}, ema5≈ema20 tol:{tol*100:.1f}%)"
            )
        if semi_bearish:
            return 'SHORT', (
                f"1D semi bearish hierarchy (Close<{ema20:.2f}<{ema50:.2f}<{sma200:.2f}, ema5≈ema20 tol:{tol*100:.1f}%)"
            )
...
def check_squeeze_quality_1h(df_1h: pd.DataFrame, config=None) -> tuple:
    ...
    # Percentile'ı erken hesapla (tüm mesajlarda görünür olması için)
    recent_bbw_pre = df_1h['bbw'].iloc[-100:]
    current_bbw_pre = df_1h.iloc[-1]['bbw']
    percentile_pre = (recent_bbw_pre < current_bbw_pre).sum() / len(recent_bbw_pre) * 100

    if squeeze_duration < min_bars:
        return False, f"Sıkışma çok kısa ({squeeze_duration} < {min_bars}) | BBW percentile:{percentile_pre:.1f}%"

    # Çok uzun squeeze durumuna opsiyonel esneklik
    semi_extended_mode = False
    extended_mode = False
    semi_ext_flag = False
    if squeeze_duration > max_bars:
        if config is not None and getattr(config, 'ENABLE_BREAKOUT_EXTENDED_SQUEEZE', False):
            ext_max = getattr(config, 'BREAKOUT_EXT_SQUEEZE_MAX_BARS', max_bars + 10)
            semi_ext_percentile_max = getattr(config, 'BREAKOUT_SEMI_EXT_BBW_PERCENTILE_MAX', 18.0)
            max_percentile_ext = getattr(config, 'BREAKOUT_EXT_BBW_PERCENTILE_MAX', 12.0)
            recent_bbw_ext = df_1h['bbw'].iloc[-100:]
            current_bbw_ext = df_1h.iloc[-1]['bbw']
            percentile_ext = (recent_bbw_ext < current_bbw_ext).sum() / len(recent_bbw_ext) * 100
            if squeeze_duration <= ext_max:
                if percentile_ext <= max_percentile_ext:
                    extended_mode = True
                    logger.debug(f"Extended Squeeze ACCEPTED: Süre={squeeze_duration}<= {ext_max}, Percentile={percentile_ext:.1f}<= {max_percentile_ext}")
                    strategy_metrics['breakout_extended_squeeze_usage'] += 1
                elif percentile_ext <= semi_ext_percentile_max:
                    semi_extended_mode = True
                    semi_ext_flag = True
                    logger.debug(f"Semi-Extended Squeeze ACCEPTED: Süre={squeeze_duration}<= {ext_max}, Percentile={percentile_ext:.1f}<= {semi_ext_percentile_max}")
                else:
                    return False, f"Sıkışma çok uzun ({squeeze_duration}>{max_bars}), percentile yüksek ({percentile_ext:.1f}% > {semi_ext_percentile_max}%)"
            else:
                return False, f"Sıkışma çok uzun ({squeeze_duration}>{ext_max}) - limit aşıldı"
        else:
            return False, f"Sıkışma çok uzun ({squeeze_duration}>{max_bars}) - enerji tükendi | BBW percentile:{percentile_pre:.1f}%"
    ...
    if percentile > bbw_percentile_max and not (extended_mode or semi_extended_mode):
        return False, f"BBW yeterince düşük değil ({percentile:.1f}% > {bbw_percentile_max:.1f}%)"

    mode_tag = "EXT" if extended_mode else ("SEMI" if semi_extended_mode else "CORE")
    squeeze_ctx = {'extended': extended_mode, 'semi_extended': semi_extended_mode, 'percentile': percentile, 'duration': squeeze_duration}
    return True, f"Squeeze {mode_tag} OK (Süre:{squeeze_duration} mum, Percentile:{percentile:.1f}%)|semi_ext={int(semi_ext_flag)}", squeeze_ctx
...
def find_pullback_signal(df_1d: pd.DataFrame, df_4h: pd.DataFrame, df_1h: pd.DataFrame, config) -> dict:
    ...
    try:
        ...
        if main_direction == 'LONG':
            ...
            # Minimal gevşetme: RSI üst sınırı 45'e çekildi (daha derin pullback arıyoruz)
            if (25 <= rsi_1h <= 45) and (macd_hist_1h <= 0):
            ...
            # SHORT tarafı şimdilik değişmedi (isteğe bağlı ayarlanabilir)
...
def find_breakout_signal(df_1d: pd.DataFrame, df_4h: pd.DataFrame, df_1h: pd.DataFrame, config) -> dict:
    ...
    if direction is None:
        ...
        # Scalp fallback önerisi: Eğer 4H momentum aşırı güçlü ise (ADX>50 ve RSI breakout LONG core üst bandına yakın)
        if df_4h is not None and len(df_4h) >= 5 and all(c in df_4h.columns for c in ['rsi14','adx14','macd_hist']):
            last4 = df_4h.iloc[-1]
            prev4 = df_4h.iloc[-2]
            if not last4[['rsi14','adx14','macd_hist']].isna().any() and not prev4[['rsi14','adx14','macd_hist']].isna().any():
                adx4 = last4['adx14']; rsi4 = last4['rsi14']; macd_hist = last4['macd_hist']; prev_macd_hist = prev4['macd_hist']
                strong_momentum = (adx4 >= 50) and (macd_hist > prev_macd_hist) and (rsi4 >= getattr(config,'BREAKOUT_RSI_CORE_HIGH',68) - 1)
                if strong_momentum and getattr(config,'ENABLE_BREAKOUT_SCALP_FALLBACK', True):
                    strategy_metrics['breakout_suggest_scalp_fallback'] += 1
                    logger.info("   💡 L1 failed fakat 4H momentum AŞIRI GÜÇLÜ → SCALP BREAKOUT fallback öneriliyor")
                    return {'direction': 'LONG', 'signal_strength': 40.0, 'strategy': 'BREAKOUT_SCALP_FALLBACK', 'note': 'L1 fail fakat 4H momentum aşırı güçlü'}
        return None
    ...
    squeeze_result = check_squeeze_quality_1h(df_1h, config)
    if isinstance(squeeze_result, tuple) and len(squeeze_result) == 3:
        squeeze_ok, squeeze_msg, squeeze_ctx = squeeze_result
    else:
        squeeze_ok, squeeze_msg = squeeze_result[0], squeeze_result[1]
        squeeze_ctx = {'extended': False, 'semi_extended': False, 'percentile': None, 'duration': None}
    if not squeeze_ok:
    ...
    # Semi/Extended sıkışmada ekstra hacim gereksinimi uygula (daha seçici)
    if vol_ok and (squeeze_ctx.get('semi_extended') or squeeze_ctx.get('extended')):
        # Ekstra tighten: semi +0.15x, extended +0.25x ratio gereksinimi
        try:
            last = df_1h.iloc[-1]
            vol = last['volume']
            avg_vol = last['volume_sma20']
            ...
            extra = (
                getattr(config, 'BREAKOUT_EXT_VOLUME_EXTRA', 0.25) if squeeze_ctx.get('extended')
                else getattr(config, 'BREAKOUT_SEMI_EXT_VOLUME_EXTRA', 0.15)
            )
            min_base = getattr(config, 'BREAKOUT_VOLUME_RATIO_MIN_RELAXED', 2.25) if getattr(config, 'ENABLE_BREAKOUT_RELAX_PHASE', False) else 2.5
            tightened_min = min_base + extra
            if ratio < tightened_min:
                vol_ok = False
                vol_msg = f"Extended türde ekstra volume şartı FAILED ({ratio:.2f}x < {tightened_min:.2f}x)"
            else:
                vol_msg += f" | TightenedOK {ratio:.2f}x>= {tightened_min:.2f}x"
        except Exception as e:
            logger.debug(f"Volume tightening hesaplanamadı: {e}")
    if not vol_ok:
    ...
    # Semi/Extended modda breakout body/distance daha sıkı olsun
    if strength_ok and (squeeze_ctx.get('semi_extended') or squeeze_ctx.get('extended')):
        try:
            last = df_1h.iloc[-1]
            prev = df_1h.iloc[-2]
            close = last['close']; open_p = last['open']; high = last['high']; low = last['low']
            bb_upper = last.get('bb_upper'); bb_lower = last.get('bb_lower')
            body = abs(close - open_p); total_range = high - low
            body_pct = (body / total_range) * 100 if total_range > 0 else 0
            if direction == 'LONG' and bb_upper and bb_upper > 0:
                dist = ((close - bb_upper) / bb_upper) * 100
            elif direction == 'SHORT' and bb_lower and bb_lower > 0:
                dist = ((bb_lower - close) / bb_lower) * 100
            else:
                dist = 0
            extra_body = (
                getattr(config, 'BREAKOUT_EXT_BODY_EXTRA', 5.0) if squeeze_ctx.get('extended')
                else getattr(config, 'BREAKOUT_SEMI_EXT_BODY_EXTRA', 3.0)
            )
            extra_dist = (
                getattr(config, 'BREAKOUT_EXT_DISTANCE_EXTRA', 0.05) if squeeze_ctx.get('extended')
                else getattr(config, 'BREAKOUT_SEMI_EXT_DISTANCE_EXTRA', 0.03)
            )
            # Temel eşikler (relax fazında iseler relaxed değerleri referans al)
            base_body = getattr(config, 'BREAKOUT_BODY_STRENGTH_MIN_RELAXED', 54.0) if getattr(config, 'ENABLE_BREAKOUT_RELAX_PHASE', False) else 60.0
            base_dist = getattr(config, 'BREAKOUT_DISTANCE_MIN_RELAXED', 0.27) if getattr(config, 'ENABLE_BREAKOUT_RELAX_PHASE', False) else 0.30
            tightened_body = base_body + extra_body
            tightened_dist = base_dist + extra_dist
            if body_pct < tightened_body or dist < tightened_dist:
                strength_ok = False
                strength_msg = f"Tightened breakout strength FAILED (Body:{body_pct:.0f}%<{tightened_body:.0f}% veya Dist:{dist:.2f}%<{tightened_dist:.2f}%)"
            else:
                strength_msg += f" | TightenedOK Body:{body_pct:.0f}%>= {tightened_body:.0f}%, Dist:{dist:.2f}%>= {tightened_dist:.2f}%"
        except Exception as e:
            logger.debug(f"Breakout tightening hesaplanamadı: {e}")
    if not strength_ok:
...
# =============================================================================
# 🆕 MEHMET KÜÇÜK STRATEJİSİ (15m-only Fast Trading System)
# =============================================================================

def find_mehmet_kucuk_signal(df_15m: pd.DataFrame, config) -> dict | None:
    """
    Mehmet Küçük Stratejisi - Basit ve hızlı 15 dakika trading
    ...
    Args:
        df_15m: 15 dakikalık DataFrame (göstergelerle)
        config: Config objesi
    
    Returns:
        {'direction': 'LONG'|'SHORT', 'signal_strength': float} veya None
    """
    required_cols = ['close', 'ema5', 'ema20', 'rsi14', 'macd_hist', 'volume', 'volume_sma20']
    
    if not validate_dataframe(df_15m, required_cols, min_rows=25):
        logger.debug("Mehmet Küçük: DataFrame validasyon başarısız")
        return None
    ...
    macd_hist = last['macd_hist']
    volume = last['volume']
    volume_avg = last['volume_sma20']
    
    # --- EMA Cross Detection ---
    ema5_prev = prev['ema5']
    ema20_prev = prev['ema20']
    
    # LONG: EMA5 방금 crossed above EMA20 VEYA hala üstte
    long_cross = (ema5 > ema20)
    # SHORT: EMA5 방금 crossed below EMA20 VEYA hala altta  
    short_cross = (ema5 < ema20)
    
    if not (long_cross or short_cross):
        logger.debug("Mehmet Küçük: EMA5/EMA20 cross yok")
        return None
    ...
    if direction == 'LONG':
        if macd_hist <= 0:
            logger.debug(f"Mehmet Küçük LONG: MACD histogram negatif ({macd_hist:.6f})")
            return None
    else:  # SHORT
        if macd_hist >= 0:
            logger.debug(f"Mehmet Küçük SHORT: MACD histogram pozitif ({macd_hist:.6f})")
            return None
    
    # --- RSI Filter (30-70 arası, aşırı bölgelerde sinyal YOK) ---
    if rsi < 30 or rsi > 70:
        logger.debug(f"Mehmet Küçük: RSI aşırı bölgede ({rsi:.1f}), sinyal reddedildi")
        return None
    
    # --- Volume Confirmation ---
    if volume_avg <= 0 or volume < volume_avg:
        logger.debug(f"Mehmet Küçük: Volume yetersiz ({volume:.0f} < {volume_avg:.0f})")
        return None
    
    vol_ratio = volume / volume_avg
    
    # --- Signal Strength Calculation ---
    # Base: 80 (basit strateji)
    # EMA spread bonus: +5 (EMA'lar arasındaki mesafe büyükse)
    # MACD strength bonus: +5 (MACD histogram güçlüyse)
    # Volume bonus: +5 (hacim yüksekse)
    # RSI optimal bonus: +5 (RSI 40-60 arası ise)
    
    signal_strength = 80.0
    
    # EMA spread
    ema_spread_pct = abs(ema5 - ema20) / ema20 * 100
    if ema_spread_pct > 0.5:  # %0.5'den fazla spread
        signal_strength += 5.0
    
    # MACD strength
    if abs(macd_hist) > 0.0001:  # Güçlü MACD
        signal_strength += 5.0
    
    # Volume
    if vol_ratio > 1.5:  # 1.5x ortalama hacim
        signal_strength += 5.0
    ...
    logger.info(f"✅ MEHMET KÜÇÜK {direction} SIGNAL | EMA5={ema5:.6f} {'>' if long_cross else '<'} EMA20={ema20:.6f} | MACD={macd_hist:.6f} | RSI={rsi:.1f} | Vol={vol_ratio:.2f}x | Strength={signal_strength:.1f}")
...
# 🆕 v10.5: 1H EMA CROSSOVER STRATEGY
def find_ema_crossover_1h_signal(df_1h: pd.DataFrame, config: object) -> dict:
    """
    1 saatlik EMA5 x EMA20 crossover stratejisi.
    
    Sinyal Mantığı:
    - BEARISH CROSSOVER (SHORT): EMA5 aşağı yönlü keser EMA20'yi
    - BULLISH CROSSOVER (LONG): EMA5 yukarı yönlü keser EMA20'yi
    
    Args:
        df_1h: 1 saatlik OHLCV + göstergeler (EMA5, EMA20)
        config: Config modülü
        
    Returns:
        dict: Sinyal varsa direction, entry, sl, tp, strategy bilgileri
              Sinyal yoksa None
    """
    if df_1h is None or df_1h.empty or len(df_1h) < 2:
        return None
    
    # EMA5 ve EMA20 kontrolü
    required_indicators = ['ema5', 'ema20', 'close', 'atr14']
    if not all(ind in df_1h.columns for ind in required_indicators):
        logger.warning("EMA crossover için gerekli göstergeler eksik (ema5, ema20)")
        return None
    ...
    # NaN kontrolü
    if pd.isna(current['ema5']) or pd.isna(current['ema20']) or \
       pd.isna(previous['ema5']) or pd.isna(previous['ema20']):
        return None
    
    # Crossover tespiti
    ema5_curr = current['ema5']
    ema20_curr = current['ema20']
    ema5_prev = previous['ema5']
    ema20_prev = previous['ema20']
    
    direction = None
    
    # BEARISH CROSSOVER (SHORT sinyal): EMA5 aşağı yönlü keser EMA20'yi
    if ema5_prev > ema20_prev and ema5_curr < ema20_curr:
        direction = 'SHORT'
        logger.info(f"   🔴 BEARISH CROSSOVER tespit edildi!")
        logger.info(f"      EMA5: {ema5_prev:.4f} > {ema20_prev:.4f} (EMA20) → {ema5_curr:.4f} < {ema20_curr:.4f}")
    
    # BULLISH CROSSOVER (LONG sinyal): EMA5 yukarı yönlü keser EMA20'yi
    elif ema5_prev < ema20_prev and ema5_curr > ema20_curr:
        direction = 'LONG'
        logger.info(f"   🟢 BULLISH CROSSOVER tespit edildi!")
        logger.info(f"      EMA5: {ema5_prev:.4f} < {ema20_prev:.4f} (EMA20) → {ema5_curr:.4f} > {ema20_curr:.4f}")
    
    # Sinyal yoksa None dön
    if direction is None:
        return None
    
    # Entry fiyatı: Mevcut kapanış fiyatı
    entry_price = float(current['close'])
    
    # ATR bazlı SL/TP hesaplama
    atr = float(current['atr14']) if not pd.isna(current['atr14']) else entry_price * 0.02
    
    # SL ve TP mesafeleri (ATR bazlı)
    sl_distance = atr * 1.5  # 1.5x ATR
    tp_distance = atr * 4.0  # 4x ATR (Risk/Reward: ~2.67)
    ...
    logger.info(f"   💰 Entry: {entry_price:.4f} | SL: {sl_price:.4f} | TP: {tp_price:.4f}")
    logger.info(f"   📊 ATR: {atr:.4f} | R:R: {rr_ratio:.2f}")
    ...
    return {
        ...
        'ema5': ema5_curr,
        'ema20': ema20_curr,
        'crossover_type': 'BEARISH' if direction == 'SHORT' else 'BULLISH'
    }
